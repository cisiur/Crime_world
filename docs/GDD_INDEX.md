...
E5-01 is complete as a documentation/specification task only. The accepted money-flow, upkeep, and transaction-ledger contract is recorded authoritatively in `BUILD_ROADMAP.md`. E5-02 is complete after bounded increments E5-02A through E5-02F: the standalone domain money-ledger foundation exists, Local Collection start cost and non-zero gross rewards use it, the recurring cost foundation exists, the first application recurring runtime exists, the crew-upkeep flow can generate schedules and execute one due period for one explicit character, and the MVP recurring-income flow can generate one schedule per organization and execute one due period. E5-03 is complete as authored content only: the six MVP business / location archetypes now have canonical immutable definitions, a `LocationKind` resolver, and focused validation in `packages/content`. E5-04 is complete as one bounded business ownership and income vertical slice: content owns three provisional business income definitions, domain owns one explicit ownership transition plus typed business-income schedule/ledger semantics, and application can execute one due business-income period for one explicit business. E5-05 is complete as a bounded recruitment-opportunity foundation: content owns four concrete persistent recruitable candidate seeds/definitions, domain owns opportunity ID/state/generation/expiration rules, and application owns a read-only visible opportunity projection. E5-06 is complete as one deterministic recruitment action: domain validates one active opportunity, records the recruitment cost through the ledger, appends membership, consumes the opportunity, and emits recruitment events; application provides a thin authored-definition wrapper. E5-07 is complete as limited crew growth and role assignment: domain owns explicit organization-member role assignments for `boss`, `operator`, and `lieutenant`; content owns canonical role-capacity values; application finalizes recruits as operators with exactly one canonical crew-upkeep schedule and can promote one operator to lieutenant for one capacity increase. Global tick-loop economy integration, business and hideout upkeep, production, inventory, storage capacity, pressure effects, rival behavior, role removal, lieutenant replacement, broader hierarchy, UI, and production UI remain unimplemented.

This is an implemented developer vertical slice plus accepted EPIC 5 financial specification, ledger, Local Collection ledger migration, recurring schedule, crew-upkeep, provisional organization-level recurring-income flows, authored MVP business / location archetype definitions, one bounded business ownership plus business-income flow, one bounded recruitment opportunity generation flow, one bounded recruitment action, and limited crew role/capacity growth with recruit upkeep synchronization. It is not final balance, a complete economy, a complete recruitment system, a complete organization hierarchy, a reusable operation catalogue, a full campaign loop, final UI, or player-facing production readiness. Campaign creation, root `GameState` operation integration, global tick-loop economy execution, save/load, pressure/investigation systems, rival AI, complete business gameplay, business upkeep, hideout upkeep, production, inventory, storage capacity, role removal, lieutenant replacement, broader hierarchy, and reusable operation catalogue expansion remain unimplemented.
...
1. review and accept E5-08 before implementation,
2. build on the accepted E5-01 money-flow, upkeep, and transaction-ledger contract plus the E5-02A through E5-02F foundation, E5-03 authored archetypes, E5-04 business-income slice, E5-05 recruitment-opportunity foundation, E5-06 recruitment action, and E5-07 limited role/capacity growth,
3. preserve the existing package boundaries, including no `packages/content` dependency from `packages/domain`,
4. keep role removal, lieutenant replacement, broader hierarchy, pressure systems, rival AI, save/load, campaign creation, and final UI pending until their accepted tasks.
