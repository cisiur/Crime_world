...
Recruitment opportunities can now be generated and expired deterministically for concrete candidates. E5-06 adds one deterministic recruitment action that uses these existing authored cost/trust values: a valid active opportunity can pay the recruitment cost through the ledger, append the candidate to the target organization, and mark the opportunity consumed. E5-07 adds the first limited crew-growth follow-up: a recruited member can receive the `operator` role, create or reuse exactly one canonical crew-upkeep schedule of `5` money every `144` ticks, and one existing operator can be promoted to `lieutenant` for exactly one operational-capacity increase. Role removal, lieutenant replacement, broader hierarchy, UI, global tick integration, save/load, and campaign integration remain unimplemented.
