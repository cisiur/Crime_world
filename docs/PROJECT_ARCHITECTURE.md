...
Current implementation:
- `packages/domain` owns stable entity IDs, deterministic simulation time, seeded randomness, root `GameState`, command/result/event foundations, the ordered tick pipeline, invariant helpers, runtime city state, district property derivation, runtime character state, runtime organization state, runtime business state, derived character availability, operation availability, planning, lifecycle, seeded outcome resolution, typed Local Collection classification, bounded consequence application, applied-consequence records, semantic operation events, and event invariants.
...
- No campaign creation, save/load, transaction ledger runtime, recurring economy execution, pressure system, recruitment system, rival AI, reusable operation catalogue, or final player-facing UI exists yet.
- For the first EPIC 4 money consequence only, domain operation planning currently updates `OrganizationState.money` directly with an explicit semantic event. E5-01 records the future transaction ledger contract in `BUILD_ROADMAP.md`; the ledger remains unimplemented and must not be preempted by a competing economy system.
- The accepted future money architecture keeps `OrganizationState.money` as the authoritative current organization balance while adding a durable append-only transaction ledger. Future successful money changes must update the balance, append exactly one ledger entry, and emit exactly one semantic money event as one atomic domain result.
...
- and no recruitment gameplay, transaction ledger, recurring economy simulation, ownership transfer, pressure system, rival AI, save/load, root campaign aggregate, or final player-facing UI was added.
...
The next architecture-sensitive task is E5-02 implementation scoping for recurring income and recurring costs. It must use the accepted E5-01 money-flow, upkeep, and transaction-ledger contract, preserve the temporary EPIC 4 direct money outcomes until migration is explicitly scoped, and avoid claiming that a complete economy, ledger runtime, save/load, pressure system, recruitment system, rival AI, or full campaign service already exists.
