...
- The content package owns the canonical MVP organization role-capacity definition: operator contribution `0`, lieutenant contribution `1`, and maximum lieutenants per organization `1`.
...
- The content package owns the canonical provisional MVP business income definitions for income-generating business archetypes only: Small Shop or Service `20` money every `144` ticks, Restaurant or Nightlife Venue `60` every `144` ticks, and Workshop or Transport Business `40` every `144` ticks.
...
Current implementation:
- `packages/domain` owns stable entity IDs, deterministic simulation time, seeded randomness, root `GameState`, command/result/event foundations, the ordered tick pipeline, invariant helpers, runtime city state, district property derivation, runtime character state, runtime organization state, runtime organization-member role assignment state/rules, runtime business state, pure business ownership transitions, derived character availability, operation availability, planning, lifecycle, seeded outcome resolution, typed Local Collection classification, bounded consequence application, applied-consequence records, ledger rules, recurring schedules, due-period recurring economy processing, crew-upkeep schedule generation, recurring-income schedule generation, business-income schedule generation, semantic domain events, and event invariants.
...
- The domain package now owns bounded deterministic operation planning through `PlanOperationCommand` and `planOperation(...)`. Planning accepts explicit runtime collections and narrow authored structural template/location inputs, validates through the existing availability evaluator, creates immutable planned operation state, reserves character assignment, reserves operational capacity, records the operation start cost through the money ledger, and emits semantic events.
...
- Consequence events include `OrganizationMoneyTransactionRecorded` for non-zero gross rewards, `CharacterPersonalExposureChanged`, `CharacterHealthChanged`, `CharacterAssignmentReleased`, `OrganizationOperationalCapacityReleased`, and `OperationConsequencesApplied`. Failure and critical-failure rewards create no zero-value money transaction or money event. Event invariants enforce requested versus actual exposure deltas, clamp semantics, injury only for critical failure, and category-consistent completion health consequence.
- `OrganizationState.money` remains the authoritative current organization balance. `MoneyTransaction` records are the authoritative financial history, and schedules plus processing records remain explicit immutable collections that are not yet attached to root `GameState`.
- E5-04 adds a bounded business ownership and income slice: `transferBusinessOwnership(...)` updates one explicit runtime business and emits `BusinessOwnershipTransferred`; `generateBusinessIncomeSchedules(...)` creates or reuses one active typed `business-income` recurring schedule per eligible owned business from caller-supplied structural income inputs; one due business income period is applied through the existing recurring economy processor and money ledger with the specific `BusinessId` as source.
- E5-07 adds bounded organization-member role assignment: explicit immutable assignment records relate a character to an organization as `boss`, `operator`, or `lieutenant`; the leader may hold `boss`; recruited ordinary members receive `operator`; one existing operator may be promoted to `lieutenant` and increase that organization's explicit `operationalCapacity` by exactly `1`. Role assignments are not stored on `CharacterState` or attached to root `GameState` yet.
...
- The application package owns thin recurring economy orchestration wrappers for one explicit due period, including controlled crew-upkeep, recurring-income, and business-income execution. These wrappers preserve domain event ordering and do not calculate authoritative balances.
- The application package owns thin E5-07 crew-growth wrappers that combine content definitions with domain state after recruitment: one finalizer assigns the recruit as an operator and creates or reuses that recruit's canonical crew-upkeep schedule, and one wrapper delegates lieutenant assignment to the domain role operation.
...
- No campaign creation, save/load, global tick-loop economy integration, business or hideout upkeep, pressure system, role removal, lieutenant replacement, broader hierarchy, rival AI, reusable operation catalogue, or final player-facing UI exists yet.
...
- E5-02C through E5-02F add the recurring economy scheduler foundation, the first application recurring runtime wrapper, deterministic crew-upkeep schedule generation plus one-period crew-upkeep execution for one explicit character, and deterministic recurring-income schedule generation plus one-period recurring-income execution for one explicit organization. This completes E5-02 as bounded recurring income and recurring cost flows. This is not global tick-pipeline integration and does not implement business gameplay.
...
The architecture review after EPIC 4 and the bounded E5-02 through E5-06 increments found the package boundaries intact:
- domain remains pure and renderer/platform independent,
- content owns immutable authored city data, rival organization seeds, operation template, outcome bands, consequence definition, canonical crew-upkeep definition, canonical recurring-income definition, canonical business / location archetype definitions, canonical business income definitions, and four canonical MVP recruitment candidate seeds/definitions with provisional costs, trust requirements, maintenance previews, locations, and durations,
- domain owns recruitment opportunity IDs, immutable runtime opportunity state, deterministic generation and expiration rules, typed failures, one deterministic recruitment action, ledger-backed recruitment payment, membership mutation, opportunity consumption, and semantic recruitment lifecycle events without importing content,
- application owns the developer playtest/session orchestration, recurring economy runtime wrapper, one-character crew-upkeep period execution, one-organization recurring-income period execution, one-business business-income period execution, the recruitment opportunity read-model projection, and a thin recruitment action runtime wrapper, and imports both domain and content,
...
- and no role removal, lieutenant replacement, broader hierarchy, global tick-loop economy integration, pressure system, rival AI, save/load, root campaign aggregate, or final player-facing UI was added.
...
E5-07 is complete as limited crew growth and role assignment, EPIC 5 remains in progress, and E5-08 is Pending as the next roadmap task. Future work must preserve the accepted E5-01/E5-02A through E5-02F, E5-04, E5-05, E5-06, and E5-07 contracts, the accepted Local Collection outcomes, the crew-upkeep, recurring-income, business-income, recruitment-opportunity, recruitment-action, and role-assignment semantics, and the content/domain boundary for authored definitions, while avoiding claims that a complete economy, complete business gameplay, complete recruitment system, complete organization hierarchy, global tick-loop integration, save/load, pressure system, rival AI, or full campaign service already exists.
