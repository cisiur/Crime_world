...
- No campaign creation, save/load, Local Collection ledger migration, recurring economy execution, pressure system, recruitment system, rival AI, reusable operation catalogue, or final player-facing UI exists yet.
- For the first EPIC 4 money consequence only, domain operation planning currently updates `OrganizationState.money` directly with an explicit semantic event. E5-01 records the future transaction ledger contract in `BUILD_ROADMAP.md`; E5-02A implements the standalone ledger foundation, but EPIC 4 code has not been migrated and must not be silently changed outside its own accepted task.
- The accepted money architecture keeps `OrganizationState.money` as the authoritative current organization balance while adding a durable append-only transaction ledger. E5-02A introduces `recordMoneyTransaction(...)` for future successful money changes to update the balance, append exactly one ledger entry, and emit exactly one semantic money event as one atomic domain result.
...
The next architecture-sensitive task is expected to be Local Collection ledger migration, subject to PM review and acceptance. It must use the accepted E5-01 contract and E5-02A foundation, preserve the temporary EPIC 4 direct money outcomes until migration is explicitly scoped, and avoid claiming that a complete economy, recurring economy execution, save/load, pressure system, recruitment system, rival AI, or full campaign service already exists.
